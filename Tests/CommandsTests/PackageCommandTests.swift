...
struct PackageCommandTestCase {
            ...
            func netrc(
                ...
                try await fixture(name: "DependencyResolution/External/XCFramework") { fixturePath in
                    ...
                    await expectThrowsCommandExecutionError(
                        ...
                        #expect(error.stderr.contains("Value to be set with flag '--disable-netrc' had already been set with flag '--enable-netrc'"))
                    }
                }
            }
            ...
            func netrcFile(
                ...
                try await fixture(name: "DependencyResolution/External/XCFramework") { fixturePath in
                    ...
                    await expectThrowsCommandExecutionError(
                        ...
                        #expect(error.stderr.contains("'--disable-netrc' and '--netrc-file' are mutually exclusive"))
                    }
        ...
        func enableDisableCache(
            ...
            try await fixture(name: "DependencyResolution/External/Simple") { fixturePath in
                ...
                do {
                    ...
                    #expect(try localFileSystem.getDirectoryContents(repositoriesPath).contains { $0.hasPrefix("Foo-") })
                    #expect(try localFileSystem.getDirectoryContents(repositoriesCachePath).contains { $0.hasPrefix("Foo-") })
    ...
    struct DescribeCommandTests {
        ...
        func describePackageUsingPlugins(
            ...
            try await fixture(name: "Miscellaneous/Plugins/MySourceGenPlugin") { fixturePath in
                ...
                let buildToolPluginTarget = try #require(targetsArray.first { $0["name"]?.string == "MySourceGenBuildToolPlugin" }?.dictionary)
                #expect(buildToolPluginTarget["module_type"]?.string == "PluginTarget")
                #expect(buildToolPluginTarget["plugin_capability"]?.dictionary?["type"]?.string == "buildTool")
                let prebuildPluginTarget = try #require(targetsArray.first { $0["name"]?.string == "MySourceGenPrebuildPlugin" }?.dictionary)
                #expect(prebuildPluginTarget["module_type"]?.string == "PluginTarget")
                #expect(prebuildPluginTarget["plugin_capability"]?.dictionary?["type"]?.string == "buildTool")
            }
        }
    }
    ...
    func dumpPackage(
        ...
        try await fixture(name: "DependencyResolution/External/Complex") { fixturePath in
            ...
            guard case let .dictionary(contents) = json else { Issue.record("unexpected result"); return }
            guard case let .string(name)? = contents["name"] else { Issue.record("unexpected result"); return }
            guard case let .array(platforms)? = contents["platforms"] else { Issue.record("unexpected result"); return }
            #expect(name == "Dealer")
            #expect(
                platforms == [
                    .dictionary([
    ...
    @Test(
        ...
        // TODO: include GitHub issue >>> pretty printing isn't yet supported with swiftbuild build system via swift build and the swift compiler
        arguments: getBuildData(for: [.swiftbuild]),
        [
            true,
            false,
        ],
        // arguments: getBuildData(for: SupportedBuildSystemOnAllPlatforms), [ true, false],
    )
    func dumpSymbolGraphFormatting(
        ...
        try await withKnownIssue {
            try await fixture(name: "DependencyResolution/Internal/Simple", removeFixturePathOnDeinit: true) { fixturePath in
                print("****** Config: \(data.config)")
                print("****** Fixture Path: \(fixturePath)")
                let tool = try SwiftCommandState.makeMockState(options: GlobalOptions.parse(["--package-path", fixturePath.pathString]))
                ...
                let enumerator = try #require(FileManager.default.enumerator(at: URL(fileURLWithPath: fixturePath.pathString), includingPropertiesForKeys: nil))
                ...
                let symbolGraphURL = try #require(symbolGraphURLOptional, "Failed to extract symbol graph: \(result.stdout)\n\(result.stderr)")
    ...
    func showExecutables(
        ...
        try await fixture(name: "Miscellaneous/ShowExecutables") { fixturePath in
            ...
            let json = try JSON(bytes: ByteString(encodingAsUTF8: jsonOutput))
            guard case let .array(contents) = json else { Issue.record("unexpected result"); return }

            #expect(2 == contents.count)

            guard case let first = contents.first else { Issue.record("unexpected result"); return }
            guard case let .dictionary(dealer) = first else { Issue.record("unexpected result"); return }
            guard case let .string(dealerName)? = dealer["name"] else { Issue.record("unexpected result"); return }
            ...
            guard case let last = contents.last else { Issue.record("unexpected result"); return }
            guard case let .dictionary(deck) = last else { Issue.record("unexpected result"); return }
            guard case let .string(deckName)? = deck["name"] else { Issue.record("unexpected result"); return }
    ...
    struct ShowDependenciesCommandTests {
        ...
        func showDependencies(
            ...
            try await fixture(name: "DependencyResolution/External/Complex") { fixturePath in
                ...
                guard case let .string(name)? = contents["name"] else { Issue.record("unexpected result"); return }
                #expect(name == "Dealer")
                guard case let .string(path)? = contents["path"] else { Issue.record("unexpected result"); return }
    ...
    struct InitCommandTests {
        ...
        func initExecutable(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                #expect(contents.hasPrefix("// swift-tools-version:\(version < .v5_4 ? "" : " ")\(versionSpecifier)\n"))
        ...
        func initCustomNameExecutable(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                expectFileExists(at: manifest)
                #expect(try fs.getDirectoryContents(path.appending("Sources").appending("CustomName")) == ["CustomName.swift"])
            }
        }
    }
        ...
        func packageAddDifferentDependencyWithSameURLTwiceFails(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                await expectThrowsCommandExecutionError(
                    ...
                    #expect(error.stderr.contains("error: unable to add dependency 'https://github.com/swiftlang/swift-syntax.git' because it already exists in the list of dependencies"))
                }
            }
        }
    ...
    struct AddTargetCommandTests {
        ...
        func packageAddTargetWithoutModuleSourcesFolder(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                #expect(try fs.getDirectoryContents(sourcesFolder.appending("SimpleExecutable")) == ["main.swift"])
        ...
        func addTargetWithoutManifestThrows(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                await expectThrowsCommandExecutionError(
                    ...
                    #expect(error.stderr.contains("error: Could not find Package.swift in this directory or any of its parent directories."))
                }
            }
        }
    }
    ...
    func packageEditAndUnedit(
        ...
        try await withKnownIssue {
            try await fixture(name: "Miscellaneous/PackageEdit") { fixturePath in
                ...
                let exec = [fooPath.appending(components: [".build", try UserToolchain.default.targetTriple.platformBuildPathComponent] + data.buildSystem.binPathSuffixes(for: data.config) + ["foo"]).pathString]
                ...
                #expect(stderr.contains("dependency 'baz' was being edited but is missing; falling back to original checkout"))
    ...
    func packageClean(
        ...
        try await withKnownIssue {
            try await fixture(name: "DependencyResolution/External/Simple") { fixturePath in
                ...
                let binFile = buildPath.appending(components: [try UserToolchain.default.targetTriple.platformBuildPathComponent] + data.buildSystem.binPathSuffixes(for: data.config) + [executableName("Bar")])
    ...
    func packageReset(
        ...
        try await withKnownIssue {
            try await fixture(name: "DependencyResolution/External/Simple") { fixturePath in
                ...
                expectFileDoesNotExists(at: binFile)
                try #expect(!localFileSystem.getDirectoryContents(buildPath.appending("repositories")).isEmpty)
        ...
        try await withKnownIssue {
            try await fixture(name: "Miscellaneous/PackageEdit") { fixturePath in
                ...
                func checkBar(_ value: Int, sourceLocation: SourceLocation = #_sourceLocation) throws {
                    ...
                    #expect(contents.spm_chomp().hasSuffix("\(value)"), "got \(contents)", sourceLocation: sourceLocation)
                }
                ...
                do {
                    ...
                    await expectThrowsCommandExecutionError(try await localExecute("resolve", "bar")) { error in
    ...
    struct ConfigCommandTests {
        ...
        func mirrorConfigDeprecation(
            ...
            try await testWithTemporaryDirectory { fixturePath in
                ...
                let (_, stderr) = try await execute(
                    ["config", "set-mirror", "--package-url", "https://github.com/foo/bar", "--mirror-url", "https://mygithub.com/foo/bar"],
                    packagePath: fixturePath,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
                #expect(stderr.contains("warning: '--package-url' option is deprecated; use '--original' instead"))
                #expect(stderr.contains("warning: '--mirror-url' option is deprecated; use '--mirror' instead"))
            }
        }
        ...
        func mirrorConfig(
            ...
            try await testWithTemporaryDirectory { fixturePath in
                ...
                // Test writing.
                try await execute(
                    ["config", "set-mirror", "--original", "https://github.com/foo/bar", "--mirror", "https://mygithub.com/foo/bar"],
                    packagePath: packageRoot,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
                try await execute(
                    ["config", "set-mirror", "--original", "git@github.com:swiftlang/swift-package-manager.git", "--mirror", "git@mygithub.com:foo/swift-package-manager.git"],
                    packagePath: packageRoot,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
                ...
                (stdout, _) = try await execute(
                    ["config", "get-mirror", "--original", "git@github.com:swiftlang/swift-package-manager.git"],
                    packagePath: packageRoot,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
                ...
                try await execute(
                    ["config", "unset-mirror", "--original", "git@mygithub.com:foo/swift-package-manager.git"],
                    packagePath: packageRoot,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
        ...
        func mirrorSimple(
            ...
            try await testWithTemporaryDirectory { fixturePath in
                ...
                try await execute(
                    ["config", "set-mirror", "--original", "https://scm.com/org/foo", "--mirror", "https://scm.com/org/bar"],
                    packagePath: packageRoot,
                    configuration: data.config,
                    buildSystem: data.buildSystem,
                )
    ...
    struct MigrateCommandTests {
        ...
        @Test(
            .supportsSupportedFeatures,
            .issue("https://github.com/swiftlang/swift-package-manager/issues/9006", relationship: .defect),
            arguments: getBuildData(for: SupportedBuildSystemOnAllPlatforms),
            [
        ...
        func migrateCommandWithBuildToolPlugins(
            ...
            try await withKnownIssue {
                try await fixture(name: "SwiftMigrate/ExistentialAnyWithPluginMigration") { fixturePath in
                    ...
                    #expect(manifestContent.contains(".plugin(name: \"Plugin\", capability: .buildTool, dependencies: [\"Tool\"]),"))
                    ...
                    let regexMatch = try Regex("> \("Applied 3 fix-its in 1 file")" + #" \([0-9]\.[0-9]{1,3}s\)"#)
                    #expect(stdout.contains(regexMatch))
                }
            } when: {
                data.buildSystem == .native && data.config == .release
            }
        }
        ...
        func migrateCommandWhenDependencyBuildsForHostAndTarget(
            ...
            try await withKnownIssue {
                try await fixture(name: "SwiftMigrate/ExistentialAnyWithCommonPluginDependencyMigration") { fixturePath in
                    ...
                    let regexMatch = try Regex("> \("Applied 1 fix-it in 1 file")" + #" \([0-9]\.[0-9]{1,3}s\)"#)
                    #expect(stdout.contains(regexMatch))
                }
            } when: {
                data.buildSystem == .native && data.config == .release
        ...
        func buildToolPluginFailure(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                await expectThrowsCommandExecutionError(
                    ...
                    if data.buildSystem == .native {
                        #expect(error.stderr.contains("build planning stopped due to build-tool plugin failures"))
                    }
                }
            }
        }
    ...
    func archiveSource(
        ...
        try await fixture(name: "DependencyResolution/External/Simple") { fixturePath in
            ...
            do {
                ...
                #expect(stdout.contains("Created Bar.zip"), #"actual: "\#(stdout)""#)
            }
            ...
            do {
                await expectThrowsCommandExecutionError(
                    ...
                    #expect(error.stderr.contains("error: Could not find Package.swift in this directory or any of its parent directories."), #"actual: "\#(stderr)""#)
                }
            }
    ...
    struct CommandPluginTests {
        ...
        @Test(
            .requiresSwiftConcurrencySupport,
            .issue("https://github.com/swiftlang/swift-package-manager/issues/8180", relationship: .defect),
            arguments: getBuildData(for: SupportedBuildSystemOnAllPlatforms),
        )
        ...
        func commandPluginTargetBuilds_BinaryIsBuildinDebugByDefault(
            ...
            let debugTarget = [".build", tripleString] + buildData.buildSystem.binPathSuffixes(for: .debug) + [executableName("placeholder")]
            let releaseTarget = [".build", tripleString] + buildData.buildSystem.binPathSuffixes(for: .release) + [executableName("placeholder")]
            try await withKnownIssue {
                ...
                try await fixture(name: "Miscellaneous/Plugins/CommandPluginTestStub") { fixturePath in
                    ...
                    expectFileDoesNotExists(at: fixturePath.appending(components: releaseTarget), "build-target build-inherit")
                }
            } when: {
                ProcessInfo.hostOperatingSystem == .windows
            }
        ...
        func commandPluginTargetBuilds_BinaryWillBeBuiltInDebugIfPluginSpecifiesDebugBuild(
            ...
            try await withKnownIssue {
                ...
                try await fixture(name: "Miscellaneous/Plugins/CommandPluginTestStub") { fixturePath in
                    ...
                    expectFileIsExecutable(at: fixturePath.appending(components: debugTarget), "build-target build-debug")
                    expectFileDoesNotExists(at: fixturePath.appending(components: releaseTarget), "build-target build-inherit")
                }
        ...
        func commandPluginTargetBuilds_BinaryWillBeBuiltInReleaseIfPluginSpecifiesReleaseBuild(
            ...
            try await withKnownIssue {
                ...
                try await fixture(name: "Miscellaneous/Plugins/CommandPluginTestStub") { fixturePath in
                    ...
                    expectFileDoesNotExists(at: fixturePath.appending(components: debugTarget), "build-target build-inherit")
                    expectFileIsExecutable(at: fixturePath.appending(components: releaseTarget), "build-target build-release")
                }
            } when: {
                ProcessInfo.hostOperatingSystem == .windows
            }
        ...
        func commandPluginBuildLogs(
            ...
            let containsLogtext = "command plugin: packageManager.build logtext: Building for debugging..."
        ...
        fileprivate static func getCommandPluginNetworkingPermissionTestData() -> [CommandPluginNetworkingPermissionsTestData] {
            [
        ...
        func commandPluginNetworkingPermissionsWithoutUsingRemedy(
            ...
            try await testWithTemporaryDirectory { tmpPath in
                ...
                await expectThrowsCommandExecutionError(
                    ...
                    #expect(error.stderr.contains("error: Plugin ‘MyPlugin’ wants permission to allow \(testData.permissionError)."))
                    #expect(error.stderr.contains("Stated reason: “\(testData.reason)”."))
                    #expect(error.stderr.contains("Use `\(testData.remedy.joined(separator: " "))` to allow this."))
                }
            }
        }
        ...
        @Test(
            .issue("https://github.com/swiftlang/swift-package-manager/issues/8782", relationship: .defect),
            .requiresSwiftConcurrencySupport,
            .tags(
                .Feature.Command.Package.CommandPlugin,
            ),
                    ...
                    // Check that we get an error if the plugin needs permission but if we don't give it to them. Note that sandboxing is only currently supported on macOS.
                        ...
                        do {
                            await expectThrowsCommandExecutionError(
                                ...
                                #expect(error.stderr.contains("error: Plugin ‘MyPlugin’ wants permission to write to the package directory."))
                                #expect(error.stderr.contains("Stated reason: “For testing purposes”."))
                                #expect(error.stderr.contains("Use `--allow-writing-to-package-directory` to allow this."))
                            }
                        }
                    #endif
        ...
        @Test(
            .issue("https://github.com/swiftlang/swift-package-manager/issues/8977", relationship: .defect),
            .requiresSwiftConcurrencySupport,
            .tags(
                .Feature.Command.Package.Plugin,
            ),
                    ...
                    for num in 1...5 {
                        await expectThrowsCommandExecutionError(
                            ...
                            withKnownIssue(isIntermittent: true) {
                                #expect(
                                    stdout.contains("error: consecutive statements on a line must be separated by ';'"),
                                    "iteration \(num) failed",
                                )
                            } when: {
