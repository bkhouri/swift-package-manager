...
extension RunCommandProtocol {
    ...
    func getBuildSystem(
        ...
        return try await swiftCommandState.createBuildSystem(
            explicitProduct: executable,
            traitConfiguration: .init(traitOptions: traitOptions),
    ...
    func execute(
        ...
        try await self.run(swiftCommandState: swiftCommandState, globalOptions: globalOptions, options: options)
        // if useBuildSystem {
        //     let buildSystem = try await getBuildSystem(
        //         swiftCommandState: swiftCommandState,
        //         executable: options.executable,
        //         traitOptions: options.traits,
        //     )
        //     let productName = try await findProductName(in: buildSystem.getPackageGraph(), options: options)
